...
import com.authorization.sample.awscognitospringauthserver.service.dto.*;
...
//    }
    ...
    @Override
    public AuthenticatedResponse updateUserPassword(UserPasswordUpdateDTO userPassword) {
        cognitoUserService.changeUserPassword(userPassword.getUsername(), userPassword.getPassword());

        // Return an appropriate response. Since we're not performing an authentication challenge,
        // we may not have tokens to return. You can modify the response as needed.
        return AuthenticatedResponse.builder()
                .username(userPassword.getUsername())
    ...
    public void resetUserPassword(PasswordResetDTO passwordResetDTO) {
        cognitoUserService.resetUserPassword(passwordResetDTO);
    }
